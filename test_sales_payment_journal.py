...
def create_test_accounts_full(db):
    ...
    db.add(acc1)
    # Piutang account
    acc2 = Account(id=uuid.uuid4(), code="1200", name="Piutang Usaha", normal_balance="debit", account_type="asset", is_active=True)
    db.add(acc2)
    # Potongan Penjualan account
    acc3 = Account(id=uuid.uuid4(), code="4200", name="Potongan Penjualan", normal_balance="debit", account_type="revenue", is_active=True)
    db.add(acc3)
    db.commit()
...
def test_create_sales_payment_journal_entry():
    # Test data with discount
    data = SalesPaymentJournalEntry(
        date=date.today(),
        memo="Test payment",
        customer_id=None,
        workorder_id=None,
    ...
    try:
        entry = create_sales_payment_journal_entry(db1, data)
        print("Test passed: Journal entry created successfully")
    ...
    data_no_discount = SalesPaymentJournalEntry(
        date=date.today(),
    ...
    try:
        entry2 = create_sales_payment_journal_entry(db2, data_no_discount)
        print("Test passed: Journal entry without discount created successfully")
...
if __name__ == "__main__":
    success = test_create_sales_payment_journal_entry()
    if success:
